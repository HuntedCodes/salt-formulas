...
JINJA = jinja2.Environment(
    loader=jinja2.FileSystemLoader(
        # Replace the libvirt_domain.jinja template with a custom one.
        os.path.join('/srv/salt/templates/virt')
...
def _disk_profile(profile, hypervisor, **kwargs):
    ...
    disklist = __salt__['config.option']('virt.disk', {}).get(
        profile, None
    )

    if disklist is None:
        disklist = __salt__['config.get']('virt:disk', {}).get(
            profile, default
        )
...
def init(name,
	...
	disk_size = diskp[0][disk_name]['size']
        ...
        qemulist = []
        qemulist.append('Disk: %s' % disk)
        disklist = __salt__['config.get']('virt:disk', {}).get(disk)
        qemulist.append('Disklist: %s' % str(disklist))

        if hypervisor in ['qemu', 'kvm']:
            # qemu-img create [-f fmt] [-o options] filename [size]
            for disk in diskp:
                for disk_name, args in disk.items():
                    disk_file = '%s.%s' % (disk_name, args['format'])
                    disk_path = os.path.join(args['pool'], name)
                    full_path = os.path.join(disk_path, disk_file)

                    if not os.path.exists(disk_path):
                        os.makedirs(disk_path)

                    cmd = ['qemu-img', 'create', '-f', args['format'],
                           full_path, str(args['size'])]

                    qemu_output = subprocess.Popen(
                        cmd, shell=False, stdout=subprocess.PIPE
                    ).communicate()[0]

            return "Profile disks: %s\n%s" % (qemu_output, str(full_path))
